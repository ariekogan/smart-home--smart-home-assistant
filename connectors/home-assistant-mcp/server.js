...
function seedDemoData() {
  ...
  if (count > 0) return;
  const upsertDevice = db.prepare(`INSERT OR REPLACE INTO devices(entity_id, domain, friendly_name, capabilities_json, selected, area_id, last_seen) VALUES(?, ?, ?, ?, 1, ?, ?)`);
  const upsertState = db.prepare(`INSERT OR REPLACE INTO device_states(entity_id, state, attributes_json) VALUES(?, ?, ?)`);
  ...
  for (const d of DEMO_DEVICES) { if (d.area_id) insertMap.run(d.area_id, d.entity_id); }
...
async function ha(pathUrl, options = {}) {
  ...
  const res = await fetch(`${HA_URL}${pathUrl}`, { ...options, headers: { Authorization: `Bearer ${HA_TOKEN}`, "Content-Type": "application/json", ...(options.headers || {}) } });
...
function demoGetStates() {
  return db.prepare(`SELECT d.entity_id, d.domain, d.friendly_name, d.area_id, ds.state, ds.attributes_json FROM devices d LEFT JOIN device_states ds ON ds.entity_id = d.entity_id`).all().map(r => ({ entity_id: r.entity_id, state: r.state || "unknown", attributes: { ...JSON.parse(r.attributes_json || "{}"), friendly_name: r.friendly_name, area_id: r.area_id, device_id: "dev_" + r.entity_id.replace(".", "_") } }));
}

function demoGetState(entityId) {
  const r = db.prepare(`SELECT d.entity_id, d.domain, d.friendly_name, d.area_id, ds.state, ds.attributes_json FROM devices d LEFT JOIN device_states ds ON ds.entity_id = d.entity_id WHERE d.entity_id = ?`).get(entityId);
...
function demoCallService(domain, service, entityId, serviceData) {
  ...
  if (entityId === "switch.bosch_oven") {
    if (service === "turn_on") {
      ...
      db.prepare("INSERT OR REPLACE INTO device_states(entity_id, state, attributes_json) VALUES(?, ?, ?)").run("sensor.bosch_oven_temp", String(attrs.target_temperature), JSON.stringify({ unit_of_measurement: "°C", manufacturer: "Bosch" }));
    } else if (service === "turn_off") {
      attrs.program = "none"; attrs.temperature = 0;
      db.prepare("INSERT OR REPLACE INTO device_states(entity_id, state, attributes_json) VALUES(?, ?, ?)").run("sensor.bosch_oven_temp", "0", JSON.stringify({ unit_of_measurement: "°C", manufacturer: "Bosch" }));
    }
  }
  db.prepare("INSERT OR REPLACE INTO device_states(entity_id, state, attributes_json) VALUES(?, ?, ?)").run(entityId, currentState, JSON.stringify(attrs));
...
function saveHCTokens(tokens) {
  db.prepare(`INSERT OR REPLACE INTO integrations(integration_id, type, status, credentials_json, config_json, updated_at) VALUES('home_connect', 'cloud_api', 'connected', ?, '{}', ?)`).run(JSON.stringify(tokens), now());
}
...
function registerHCAppliance(appliance) {
  ...
  db.prepare(`INSERT OR REPLACE INTO devices(entity_id, domain, friendly_name, capabilities_json, selected, area_id, last_seen) VALUES(?, ?, ?, ?, 1, ?, ?)`).run(entityId, domain, appliance.name || `${appliance.brand} ${appliance.type}`, JSON.stringify(caps), "kitchen", now());
  ...
  db.prepare("INSERT OR REPLACE INTO setup_state(key, value_json) VALUES(?, ?)").run(`hc_mapping_${entityId}`, JSON.stringify({ haId: appliance.haId, brand: appliance.brand, type: appliance.type, vib: appliance.vib }));
...
function tvRequest(uri, payload = {}) {
  return new Promise((resolve, reject) => {
    ...
    tvClient.request(uri, payload, (err, res) => { if (err) reject(err); else resolve(res); });
  });
}
function tvSubscribeOnce(uri) {
  return new Promise((resolve, reject) => {
    if (!tvClient || !tvConnected) return reject(new Error("TV not connected"));
    const unsub = tvClient.subscribe(uri, (err, res) => { if (unsub) try { unsub(); } catch {} if (err) reject(err); else resolve(res); });
    setTimeout(() => reject(new Error("TV subscribe timeout")), 5000);
  });
}

async function getTVState() {
  if (!tvConnected) return { entity_id: TV_ENTITY_ID, state: "off", attributes: { friendly_name: "LG TV", device_class: "tv", manufacturer: "LG", ip: LG_TV_IP || "unknown" } };
  try {
    const [volume, app, inputs] = await Promise.allSettled([tvSubscribeOnce("ssap://audio/getVolume"), tvSubscribeOnce("ssap://com.webos.applicationManager/getForegroundAppInfo"), tvRequest("ssap://tv/getExternalInputList")]);
    ...
    return { entity_id: TV_ENTITY_ID, state: "on", attributes: { friendly_name: db.prepare("SELECT friendly_name FROM devices WHERE entity_id = ?").get(TV_ENTITY_ID)?.friendly_name || "LG TV", device_class: "tv", manufacturer: "LG", ip: LG_TV_IP, volume_level: vol.volume ?? null, is_muted: vol.muted ?? false, current_app: fg.appId || "unknown", source_list: (inp.devices || []).map(d => d.id), media_controls: ["play", "pause", "stop", "rewind", "fastForward"] } };
  } catch {
    return { entity_id: TV_ENTITY_ID, state: "on", attributes: { friendly_name: "LG TV", device_class: "tv", manufacturer: "LG", ip: LG_TV_IP } };
  }
}

async function callTVService(service, serviceData = {}) {
  if (!tvConnected && service !== "turn_on" && LG_TV_IP) { try { await connectTV(LG_TV_IP); } catch { throw new Error("TV is off or not reachable. Use turn_on first."); } }
  if (!tvConnected && service !== "turn_on") throw new Error("TV is off or not connected. Use turn_on first.");
  switch (service) {
    case "turn_on": {
      ...
      if (LG_TV_MAC) {
        ...
        if (LG_TV_IP && !tvConnected) {
          ...
          try { await connectTV(LG_TV_IP); console.error("[lgtv] Reconnected after WoL!"); return [{ entity_id: TV_ENTITY_ID, state: "on", attributes: { friendly_name: "LG TV" } }]; } catch (e) { console.error("[lgtv] Reconnect after WoL failed, TV may still be booting: " + e.message); }
        }
    ...
    case "turn_off": await tvRequest("ssap://system/turnOff"); tvConnected = false; return [{ entity_id: TV_ENTITY_ID, state: "off", attributes: { friendly_name: "LG TV" } }];
    case "volume_set": await tvRequest("ssap://audio/setVolume", { volume: serviceData.volume_level ?? serviceData.volume ?? 20 }); return [{ entity_id: TV_ENTITY_ID, state: "on", attributes: { friendly_name: "LG TV", volume_level: serviceData.volume_level ?? serviceData.volume } }];
    case "volume_up": await tvRequest("ssap://audio/volumeUp"); return [{ entity_id: TV_ENTITY_ID, state: "on", attributes: { friendly_name: "LG TV" } }];
    case "volume_down": await tvRequest("ssap://audio/volumeDown"); return [{ entity_id: TV_ENTITY_ID, state: "on", attributes: { friendly_name: "LG TV" } }];
    case "mute": await tvRequest("ssap://audio/setMute", { mute: serviceData.mute ?? true }); return [{ entity_id: TV_ENTITY_ID, state: "on", attributes: { friendly_name: "LG TV", is_muted: serviceData.mute ?? true } }];
    case "select_source": await tvRequest("ssap://tv/switchInput", { inputId: serviceData.source }); return [{ entity_id: TV_ENTITY_ID, state: "on", attributes: { friendly_name: "LG TV", source: serviceData.source } }];
    case "launch_app": await tvRequest("ssap://system.launcher/launch", { id: serviceData.app_id }); return [{ entity_id: TV_ENTITY_ID, state: "on", attributes: { friendly_name: "LG TV", current_app: serviceData.app_id } }];
    case "media_play": await tvRequest("ssap://media.controls/play"); return [{ entity_id: TV_ENTITY_ID, state: "on" }];
    case "media_pause": await tvRequest("ssap://media.controls/pause"); return [{ entity_id: TV_ENTITY_ID, state: "on" }];
    case "send_notification": await tvRequest("ssap://system.notifications/createToast", { message: serviceData.message || "Hello!" }); return [{ entity_id: TV_ENTITY_ID, state: "on" }];
    default: throw new Error(`Unknown TV service: ${service}`);
  }
}

async function detectTVModel() {
  try { const sys = await tvRequest("ssap://system/getSystemInfo"); const sw = await tvRequest("ssap://com.webos.service.update/getCurrentSWInformation"); return { model: sys?.modelName || "LG TV", serial: sys?.serialNumber || null, webos: sw?.product_name || null, mac: sw?.device_id || LG_TV_MAC || null, country: sw?.country || null }; } catch { return { model: "LG TV" }; }
}
...
function registerTVEntity(tvInfo = {}) {
  ...
  db.prepare(`INSERT OR REPLACE INTO devices(entity_id, domain, friendly_name, capabilities_json, selected, area_id, last_seen) VALUES(?, ?, ?, ?, 1, ?, ?)`).run(TV_ENTITY_ID, "media_player", friendlyName, JSON.stringify(caps), "living_room", now());
...
async function connectTV(ip) {
  ...
  return new Promise((resolve, reject) => {
    const client = lgtv2({ url: `ws://${ip}:3000`, keyFile: TV_KEY_FILE, reconnect: false });
    const timeout = setTimeout(() => { tvReconnecting = false; reject(new Error("TV connection timeout — is the TV on and on the same network?")); }, 20000);
    client.on("prompt", () => { console.error("[lgtv] >>> Accept the connection on your TV screen <<<"); });
    client.on("connect", async () => { clearTimeout(timeout); tvClient = client; tvConnected = true; tvReconnecting = false; const tvInfo = await detectTVModel(); registerTVEntity(tvInfo); console.error("[lgtv] Connected to LG " + tvInfo.model + " at " + ip); resolve({ connected: true, ip, entity_id: TV_ENTITY_ID, model: tvInfo.model, webos: tvInfo.webos, mac: tvInfo.mac }); });
    client.on("close", () => { tvConnected = false; console.error("[lgtv] TV disconnected"); });
    client.on("error", (err) => { console.error("[lgtv] Error: " + (err?.message || err)); });
  });
}

if (LG_TV_IP) console.error("[lgtv] TV configured at " + LG_TV_IP + " — will connect on first use");

// --- Business logic ------------------------------------------------------
function configSummary() {
  const devices = db.prepare("SELECT * FROM devices ORDER BY domain, friendly_name").all().map(r => ({ entity_id: r.entity_id, domain: r.domain, friendly_name: r.friendly_name, capabilities: JSON.parse(r.capabilities_json || "[]"), selected: !!r.selected, area_id: r.area_id || null, last_seen: r.last_seen }));
  return { version: "0.3.0-dev", devices, updated_at: now() };
}

function roomsList() {
  const rows = db.prepare(`SELECT r.room_id, r.name, rd.entity_id, d.friendly_name FROM rooms r LEFT JOIN room_devices rd ON rd.room_id = r.room_id LEFT JOIN devices d ON d.entity_id = rd.entity_id ORDER BY r.name, d.friendly_name`).all();
  const out = {};
...
async function syncRooms() {
  if (DEMO_MODE) return roomsList();
  let areas = [], registryDevices = [], states = [];
  try { areas = await ha("/api/config/area_registry/list", { method: "POST", body: "{}" }); registryDevices = await ha("/api/config/device_registry/list", { method: "POST", body: "{}" }); states = await ha("/api/states"); } catch { return roomsList(); }
  db.prepare("DELETE FROM rooms").run(); db.prepare("DELETE FROM room_devices").run();
  const insertRoom = db.prepare("INSERT INTO rooms(room_id, name) VALUES(?, ?)");
  const insertMap = db.prepare("INSERT OR IGNORE INTO room_devices(room_id, entity_id) VALUES(?, ?)");
  const upsertDevice = db.prepare(`INSERT OR REPLACE INTO devices(entity_id, domain, friendly_name, capabilities_json, selected, area_id, last_seen) VALUES(?, ?, ?, ?, COALESCE((SELECT selected FROM devices WHERE entity_id = ?), 0), ?, ?)`);
  ...
  for (const s of states) {
    ...
    upsertDevice.run(s.entity_id, String(s.entity_id || "").split(".")[0], attrs.friendly_name || s.entity_id, JSON.stringify(deriveCapabilities(s.entity_id)), s.entity_id, areaId, now());
...
async function entitiesList() {
  if (DEMO_MODE) return configSummary().devices;
  const states = await ha("/api/states");
  const stmt = db.prepare(`INSERT OR REPLACE INTO devices(entity_id, domain, friendly_name, capabilities_json, selected, area_id, last_seen) VALUES(?, ?, ?, ?, COALESCE((SELECT selected FROM devices WHERE entity_id = ?), 0), COALESCE((SELECT area_id FROM devices WHERE entity_id = ?), NULL), ?)`);
  for (const s of states) { stmt.run(s.entity_id, String(s.entity_id || "").split(".")[0], s.attributes?.friendly_name || s.entity_id, JSON.stringify(deriveCapabilities(s.entity_id)), s.entity_id, s.entity_id, now()); }
...
async function callTool(name, args = {}) {
  switch (name) {
    case "entities.list": {
      ...
      if (LG_TV_IP) { const exists = list.some(d => d.entity_id === TV_ENTITY_ID); if (!exists) list.push({ entity_id: TV_ENTITY_ID, domain: "media_player", friendly_name: "LG TV", capabilities: ["read_state", "turn_on", "turn_off", "volume_set", "volume_up", "volume_down", "mute", "select_source", "launch_app"], selected: true, area_id: "living_room", last_seen: now() }); }
      return list;
    }
    case "entity.state": {
    ...
    case "services.call": {
      let result;
      if (args.entity_id === TV_ENTITY_ID || args.domain === "media_player") { result = await callTVService(args.service, args.service_data); }
      else if (DEMO_MODE) { result = demoCallService(args.domain, args.service, args.entity_id, args.service_data); }
      ...
      db.prepare("INSERT INTO test_runs(id, entity_id, action, status, created_at) VALUES(?, ?, ?, ?, ?)").run(`test_${Date.now()}`, args.entity_id || TV_ENTITY_ID, `${args.domain}.${args.service}`, "ok", now());
      return result;
    }
    case "tv.discover": {
      ...
      try { return await connectTV(discoveredIP); } catch (e) { return { connected: false, error: e.message }; }
    }
    case "tv.apps": { if (!tvConnected) return { error: "TV not connected" }; const res = await tvRequest("ssap://com.webos.applicationManager/listLaunchPoints"); return { apps: (res.launchPoints || []).map(a => ({ id: a.id, title: a.title })) }; }
    case "tv.inputs": { if (!tvConnected) return { error: "TV not connected" }; const res = await tvRequest("ssap://tv/getExternalInputList"); return { inputs: (res.devices || []).map(d => ({ id: d.id, label: d.label, connected: d.connected })) }; }
    case "discovery.supported": {
      return { integrations: [
        { id: "home_connect", name: "Bosch / Siemens Home Connect", type: "cloud_api", status: getHCIntegration()?.status || "disconnected", devices: ["Ovens", "Dishwashers", "Washers", "Dryers", "Coffee Machines"], setup: "OAuth2 — user authorizes via Home Connect website, then provides the code back in chat", requires: "Home Connect account + app paired with appliance" },
        { id: "lgtv", name: "LG WebOS TV", type: "local_network", status: tvConnected ? "connected" : "disconnected", devices: ["LG Smart TVs (2014+)"], setup: "Local network — connector discovers TV, user accepts pairing on TV screen", requires: "TV on same network, 'LG Connect Apps' enabled in TV settings" },
    ...
    case "discovery.status": {
      const integrations = db.prepare("SELECT * FROM integrations").all().map(r => ({ id: r.integration_id, type: r.type, status: r.status, updated_at: r.updated_at }));
      return { integrations, tv: { connected: tvConnected, ip: LG_TV_IP }, homeassistant: { connected: !!HA_URL, url: HA_URL || null }, demo_mode: DEMO_MODE, total_devices: db.prepare("SELECT COUNT(*) as c FROM devices").get().c };
    }
    case "homeconnect.auth": {
      ...
      return { action: "Please open this URL in your browser and authorize the connection:", auth_url: authUrl, next_step: "After authorizing, you'll be redirected. Copy the 'code' parameter from the redirect URL and provide it using the homeconnect.callback tool.", note: "If you don't have a Home Connect account yet, download the Home Connect app and pair your Bosch appliance first." };
    }
    case "homeconnect.callback": {
      ...
      if (!tokenRes.ok) { const errText = await tokenRes.text(); return { error: `Authorization failed: ${errText}` }; }
      const tokens = await tokenRes.json();
      tokens.expires_at = Date.now() + (tokens.expires_in || 86400) * 1000;
      saveHCTokens(tokens);
      try { const appliances = await hcFetch("/api/homeappliances"); const registered = (appliances.data?.homeappliances || []).map(registerHCAppliance); return { connected: true, appliances: registered }; } catch { return { connected: true, appliances: [], note: "Connected but couldn't fetch appliances yet. Try homeconnect.appliances." }; }
    }
    case "homeconnect.appliances": { const data = await hcFetch("/api/homeappliances"); return { appliances: (data.data?.homeappliances || []).map(registerHCAppliance) }; }
    case "homeconnect.status": { if (!args.ha_id) return { error: "Provide ha_id (appliance ID)" }; const data = await hcFetch(`/api/homeappliances/${encodeURIComponent(args.ha_id)}/status`); return data.data || data; }
    case "homeconnect.programs": { if (!args.ha_id) return { error: "Provide ha_id (appliance ID)" }; const data = await hcFetch(`/api/homeappliances/${encodeURIComponent(args.ha_id)}/programs/available`); return data.data || data; }
    case "homeconnect.start_program": {
      ...
      await hcFetch(`/api/homeappliances/${encodeURIComponent(args.ha_id)}/programs/active`, { method: "PUT", body: JSON.stringify({ data: { key: args.program_key, options } }) });
      return { started: true, program: args.program_key, temperature: args.temperature, duration: args.duration };
    }
    case "homeconnect.stop_program": { if (!args.ha_id) return { error: "Provide ha_id" }; await hcFetch(`/api/homeappliances/${encodeURIComponent(args.ha_id)}/programs/active`, { method: "DELETE" }); return { stopped: true }; }
    case "config.get": return configSummary();
    case "config.save": { for (const device of (args.config?.devices || [])) { db.prepare("UPDATE devices SET selected = ? WHERE entity_id = ?").run(device.selected ? 1 : 0, device.entity_id); } return { ok: true, ...configSummary() }; }
    case "rooms.list": return roomsList();
    case "rooms.sync": return await syncRooms();
    case "ui.listPlugins":
      return { plugins: [
    ...
    case "ui.getPlugin": {
      ...
      return { id: "home-layout-panel", name: "Home Layout Panel", version: "1.0.0", description: "Smart home dashboard with rooms, devices, quick controls, and multi-provider status", iframeUrl: "/ui/home-layout-panel/1.0.0/index.html", render: { mode: "iframe", iframeUrl: "/ui/home-layout-panel/1.0.0/index.html" }, channels: ["command"], capabilities: { commands: [] } };
    }
...
process.stdin.on("data", async (chunk) => {
  ...
  while ((idx = buffer.indexOf("\n")) >= 0) {
    ...
    try {
      if (method === "initialize") { writeResult(id, { protocolVersion: "2024-11-05", capabilities: { tools: {} }, serverInfo: { name: "home-assistant-mcp", version: "0.5.0" } }); continue; }
